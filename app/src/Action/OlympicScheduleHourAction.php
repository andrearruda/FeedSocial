...
final class OlympicScheduleHourAction
{
    /*
     * URL
    ...
    public function __invoke(Request $request, Response $response, $args)
    {
        ...
        $key = FileSystemCache::generateCacheKey('schedule.hour', null);
            ...
            foreach($events as $i => $event)
            {
                foreach($event['itens'] as $j => $category)
                {
                    foreach($category['itens'] as $k => $item)
                    {
                         ...
                         */
                        $dateTimeItem = Carbon::createFromFormat('d/m/Y H:i', $item['data'] . ' ' . str_replace('h', ':', $item['hora']));
                        $dateInterval = $this->dateTimeCurrent->diff($dateTimeItem);

                        if($dateInterval->h == 0 && $dateInterval->invert == 1)
                        {
